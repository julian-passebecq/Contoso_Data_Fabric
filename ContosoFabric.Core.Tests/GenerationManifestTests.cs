...
{
    [Fact]
    public async Task Generator_log_is_converted_to_structured_truth_manifest()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(
                ...
                "INFO Orders: 1000\nINFO OrdersRows: 2741\nINFO Orders: 12,345\nINFO OrdersRows: 34,567\nTHE END\n");
    ...
    public async Task Manifest_creation_fails_when_generator_final_counters_are_missing()
    {
        ...
        try
        {
            await File.WriteAllTextAsync(Path.Combine(folder, "_log.log"), "START\nTHE END\n");
            var project = new FabricProject(
                Name: "manifest-test",
                Scenario: BusinessScenario.SalesBi,
                Scale: DataScale.Tiny,
